...
import { useLayoutPresetsStore } from '../../stores/layout-presets'
...
export function SavePresetDialog({ isOpen, onClose, onSaved }: SavePresetDialogProps) {
  const [name, setName] = useState('')
  const [error, setError] = useState<string | null>(null)
  const inputRef = useRef<HTMLInputElement>(null)

  const saveCurrentAsPreset = useLayoutPresetsStore((state) => state.saveCurrentAsPreset)
      ...
      <div className="absolute inset-0 flex items-center justify-center p-8">
        <div className="w-full max-w-sm bg-background-secondary border border-border rounded-lg shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <h2 className="text-base font-semibold text-slate-100">Save Current Layout</h2>
            <button
              onClick={onClose}
          ...
          <div className="p-4 space-y-4">
            <p className="text-sm text-slate-400">
              Save the current panel sizes and layout as a custom preset.
            </p>

            {/* Name input */}
            <div>
              <label htmlFor="preset-name" className="block text-sm font-medium text-slate-200 mb-1.5">
